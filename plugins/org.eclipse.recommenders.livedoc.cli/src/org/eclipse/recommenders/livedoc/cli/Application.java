...
public class Application implements IApplication {
    ...
    public Object start(IApplicationContext context) throws Exception {
        ...
        RepositoryDescriptor sourceRepositoryDescriptor = new RepositoryDescriptor("sourceRepo", settings.getSourceRepo());
        ...
        //TODO: ugly...
        ArrayList<String> subpackages = Lists.newArrayListWithCapacity(1);
        subpackages.add(artifact.getGroupId());
        
        File livedocDir = new File(TEMP_DIR + File.separator + "livedoc_tmp");
        
        ILiveDoc livedoc = new LiveDoc(settings.isVerbose(), sourceFiles, livedocDir, subpackages);
        livedoc.generate();
        ...
        if (settings.getOutputDir() != null){
            ...
            FileUtils.copyDirectory(livedocDir, output);
        }
     
        sb.append(".jar");
        
        
        File tmpOutput = new File(TEMP_DIR + File.separator + sb.toString());
        ZipUtils.zip(livedocDir, tmpOutput);
        ...
        if (settings.getUploadRepo() != null){
            Artifact uploadArtifact = new DefaultArtifact(artifact.getGroupId(), artifact.getArtifactId(),"javadoc", "jar", artifact.getVersion());
            uploadArtifact = uploadArtifact.setFile(tmpOutput);
            RepositoryDescriptor uploadRepositoryDescriptor = new RepositoryDescriptor("uploadRepo", settings.getUploadRepo());
